...
CVector4D CVector4D::operator + (const CVector4D& b) const { return CVector4D(v[0] + b[0], v[1] + b[1], v[2] + b[2], v[3] + b[3]); }
CVector4D CVector4D::operator - (const CVector4D& b) const { return CVector4D(v[0] - b[0], v[1] - b[1], v[2] - b[2], v[3] - b[3]); }
CVector4D CVector4D::operator * (const CVector4D& b) const { return CVector4D(v[0] * b[0], v[1] * b[1], v[2] * b[2], v[3] * b[3]); }
CVector4D CVector4D::operator / (const CVector4D& b) const {
	if (b[0] == 0.0f || b[1] == 0.0f || b[2] == 0.0f || b[3] == 0.0f) { return CVector4D(); }
	return CVector4D(v[0] / b[0], v[1] / b[1], v[2] / b[2], v[3] / b[3]); 
}

CVector4D operator + (const float& v, const CVector4D& b) { return b + v;  }
CVector4D operator - (const float& v, const CVector4D& b) { return -b + v; }
CVector4D operator * (const float& v, const CVector4D& b) { return b * v;  }
CVector4D operator / (const float& v, const CVector4D& b) {
	if (b[0] == 0.0f || b[1] == 0.0f || b[2] == 0.0f || b[3] == 0.0f) { return CVector4D(); }
	return CVector4D(v / b[0], v / b[1], v / b[2], v / b[3]);
}
